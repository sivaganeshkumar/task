...
if (isset($_POST["username"])) 
{
  ...
  $l_tname = $_POST["lastname"];
  $p_dob = $_POST["dob"];
  $p_gender = $_POST["gender"];
  $p_email = $_POST["email"];
  $p_phone = $_POST["phone"];
  $p_martialstatus = $_POST["martialstatus"];
  $p_userimage = $_POST["userimage"];
  $p_hobby= $_POST["hobby"];
 
  $sql = "SELECT username FROM profile WHERE username='$f_name'";
  $result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0)
 {
         $fmsg="your profile ha already created!";
        
  }
    else {
        
  $sql= "INSERT INTO profile (id,firstname,lastname,dob,gender,biodata,email,phone,maritalstatus,userimage,hobby) VALUES ('','$f_name'.'$l_name','$p_dob','$p_gender','$p_biodata','$p_email','$p_phone',$p_maritalstatus','$p_userimage','$p_hobby')";
  $result=mysqli_query($connection,$sql);
             if($result)
             {
               $smsg="profile created Successfull";
             }
             else
             {
 ...
 <link  rel="stylsheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
   ...
   <nav class="navbar navbar-default">
      ...
      <div class="col-md-1 pull-right">      
               <a href="index.html" class="btn btn-info" role="button">Logout</a>
      </div>
    </nav>
